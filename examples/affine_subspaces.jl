...
cons_ref = @constraint(model, s .== [-1.5, -1.5])
...
# vertex 1 : y = 0
con_ref = @constraint(model, q_0[1,2] == 0.)
MOI.set(model, ConstraintVertexOrEdge(), con_ref, 2)
con_ref = @constraint(model, q_1[1,2] == 0.)
MOI.set(model, ConstraintVertexOrEdge(), con_ref, 2)

cost = @variable(model)
MOI.set(model, VariableVertexOrEdge(), cost, 2)
set_vertex_or_edge_objective(model, 2, 10.0cost)
con_ref = @constraint(model, [cost; q_1[1,:] - q_0[1,:]] in SecondOrderCone())
MOI.set(model, ConstraintVertexOrEdge(), con_ref, 2)

# vertex 2 : y = 3x + 3
con_ref = @constraint(model, 3q_0[2,1] - q_0[2,2] == -3.)
MOI.set(model, ConstraintVertexOrEdge(), con_ref, 3)
con_ref = @constraint(model, 3q_1[2,1] - q_1[2,2] == -3.)
MOI.set(model, ConstraintVertexOrEdge(), con_ref, 3)

cost = @variable(model)
MOI.set(model, VariableVertexOrEdge(), cost, 3)
set_vertex_or_edge_objective(model, 3, cost)
con_ref = @constraint(model, [cost; q_1[2,:] - q_0[2,:]] in SecondOrderCone())
MOI.set(model, ConstraintVertexOrEdge(), con_ref, 3)
...
set_vertex_or_edge_objective(model, 4, cost)
con_ref = @constraint(model, [cost; q_1[3,:] - q_0[3,:]] in SecondOrderCone())
MOI.set(model, ConstraintVertexOrEdge(), con_ref, 4)

# target
@variable(model, t[1:2])
MOI.set.(model, VariableVertexOrEdge(), t, 5)
cons_ref = @constraint(model, t .== [1.5, -1.5])
MOI.set.(model, ConstraintVertexOrEdge(), cons_ref, 5)

# Edges with source and target
cons_ref = @constraint(model, s .== q_0[2,:])
MOI.set(model, ConstraintVertexOrEdge(), cons_ref[1], (1, 3))
MOI.set(model, ConstraintVertexOrEdge(), cons_ref[2], (1, 3))
